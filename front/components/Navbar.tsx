...
export const Navbar = () => {
  ...
  // If not scrolled and on a dark hero, text should be white.
  // If scrolled or in light mode without hero, text should be foreground.
  const textColorClass = isScrolled ? "text-foreground" : "text-white";

  return (
    <>
      <nav 
        className={`fixed top-0 left-0 w-full z-50 transition-all duration-500 flex flex-row items-center justify-between px-6 py-4 md:px-20 lg:px-40 ${
          isScrolled 
      ...
      <AnimatePresence>
        {isOpen && (
          <motion.div
            ...
            className="fixed inset-0 z-[60] bg-rm-blue-dark flex flex-col p-10"
          >
            ...
            <ul className="flex flex-col gap-8">
              {navLinks.map((link) => (
                ...
                >
                  <a
                    href={link.href}
                    className="text-5xl font-black uppercase text-white hover:text-rm-gold transition-all duration-300 group-hover:italic"
                  >
                    {link.name}
                  </a>
                </motion.li>
