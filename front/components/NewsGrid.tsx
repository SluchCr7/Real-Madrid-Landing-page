...
import useNews, { NewsArticleType } from "@/hooks/useNews";

const fallbackNewsData: NewsArticleType[] = [
  {
    id: "fb-1",
...
export const NewsGrid = () => {
  const { news } = useNews();
  const displayNews = news.length > 5 ? news : fallbackNewsData;
  ...
  return (
    <section className="w-full py-60 bg-background transition-colors duration-500 overflow-hidden relative">
      ...
      <div className="max-w-[1800px] mx-auto px-4 md:px-20 flex flex-col gap-24 relative z-10">
        ...
        <div className="grid w-full gap-10 lg:grid-cols-[2fr_1fr]">
          {/* Big Featured News */}
          <a
            href={displayNews[0]?.url || "#"}
            target="_blank"
          ...
          >
            <motion.div
              ...
              viewport={{ once: true }}
              className="group w-full relative h-[700px] rounded-[60px] overflow-hidden shadow-premium border border-border-subtle bg-background/50"
            >
              <img
              ...
              <div className="absolute inset-0 bg-gradient-to-t from-rm-blue-dark via-rm-blue-dark/40 to-transparent p-16 flex flex-col justify-end">
                <div className="flex items-center gap-6 mb-6">
                  <span className="px-4 py-1.5 bg-rm-gold text-rm-blue-dark text-[10px] font-black uppercase tracking-[0.2em] italic rounded-lg shadow-gold">
                  ...
                  <div className="flex items-center gap-3 text-white/80 text-[10px] font-black uppercase tracking-widest">
                    ...
                    <span>{displayNews[0]?.date}</span>
                  </div>
                </div>

                <h3 className="text-3xl md:text-5xl font-black uppercase text-white tracking-tighter leading-none italic group-hover:text-gold transition-colors underline-offset-8 group-hover:underline line-clamp-4">
                  {displayNews[0]?.title}
                </h3>
              </div>
          ...
          {/* Side Small News */}
          <div className="grid grid-rows-2 gap-10 w-full h-full">
            {displayNews.slice(1, 3).map((news) => (
              ...
              >
                <motion.div
                  ...
                  whileInView={{ opacity: 1, x: 0 }}
                  viewport={{ once: true }}
                  className="group relative h-full rounded-[50px] w-full overflow-hidden shadow-premium border border-border-subtle transition-all hover:border-rm-gold/40 bg-background/50"
                >
                  <img
                    src={news.image}
                    alt={news.title}
                  ...
                  <div className="absolute inset-0 bg-gradient-to-t from-black/80 via-black/20 to-transparent p-10 flex flex-col justify-end">
                    <div className="flex items-center justify-between mb-4">
                      <span className="text-rm-gold text-[10px] font-black uppercase tracking-widest italic">
                        {news.category}
                      </span>
                      <TrendingUp className="w-4 h-4 text-rm-gold/40" />
                    </div>

                    <h3 className="text-xl md:text-2xl font-black uppercase text-white tracking-tighter leading-none italic group-hover:translate-x-2 transition-transform duration-500 line-clamp-3">
                      {news.title}
                    </h3>
                  </div>
                </motion.div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 w-full gap-12">
          {displayNews.slice(3, 6).map((news, idx) => (
            <a href={news.url || "#"} target="_blank" rel="noopener noreferrer" key={news.id} className="block w-full">
              <motion.div
              ...
              >
                <div className="h-[300px] w-full relative rounded-[40px] overflow-hidden shadow-premium border border-border-subtle group-hover:border-rm-gold/30 transition-all bg-background/50">
                  <img src={news.image} alt={news.title} className="absolute inset-0 w-full h-full object-cover group-hover:scale-110 transition-transform duration-1000" />
                ...
                <div className="flex flex-col gap-4 px-2">
                  ...
                  <h4 className="text-2xl font-black uppercase text-foreground tracking-tighter leading-none italic group-hover:text-rm-gold transition-colors duration-300 line-clamp-3">{news.title}</h4>
                </div>
              </motion.div>
            </a>
          ))}
